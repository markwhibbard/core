            ...
            if len(self._discovereddevices) > 0:
                for device in self._discovereddevices:
                    if device['DeviceType'][0:4].lower() == "pico":
                        entity_id = "mwh_lutron_pico_" + device['href'].split('/device/')[1]
                        name = device['FullyQualifiedName'][0] + "_" + device['FullyQualifiedName'][1]
    ...
    async def connect(self):
        def message_received(topic, payload, qos):
            ...
            if dowhat=="scenes":
                ...
                print("Discovered {} lutron scenes:".format(len(self._discoveredscenes)))
                return
            if dowhat=="devices":
                #Handle incoming device discovery
                self._discovereddevices = json.loads(payload)
                print("Discovered {} lutron devices:".format(len(self._discovereddevices)))
                return
            if dowhat=="state":
                        ...
                        if "scenes" in status.keys():
                            for scene_id, state in status['scenes'].items():
                                ...
                                if thisscene is None:
                                    print("Error in mwh lutron: didn't find scene with id " + entity_id)
                                else:
#                                    print(f"Matched incoming scene_id {scene_id} with scene named: {thisscene.name} with state {thisscene.is_on}")
                        ...
                        if "action" in status.keys():
                            entity_id = "mwh_lutron_pico_" + status['device']
                            button = status['button']
                            thispico = None
                            for pico in self._picos:
                                if pico.entity_id == entity_id:
                                    if thispico is not None:
                                        print("Error in mwh lutron: found duplicate pico id")
                                    else:
                                        thispico=pico
                            if thispico is None:  
                                if entity_id != 'mwh_lutron_pico_1': #device 1 is the hub, ignore those
                                    print("Error in mwh lutron: didn't find pico with id " + entity_id)
                            else:
        ...
        await self._hass.components.mqtt.async_subscribe(self._topic + "/lutron/#", message_received)
...
class SceneDevice(LutronDevice):
    ...
    def turnon(self):
        self.hub._hass.components.mqtt.async_publish(
            ...
            json.dumps({"virtualButton": self._entity_id.split('mwh_lutron_scene_')[1]}))
        self.hub._hass.components.mqtt.async_publish(
    ...
    def turnoff(self):
        ...
        for scene in self.hub._scenes:
            ...
            if firstword == roomname and (lastword.lower() == "dark" or lastword.lower() == "off"):
                scene.turnon()
                return True
...
class PicoDevice(LutronDevice):
    ...
    def __init__(self, entity_id, name, picotype, hub):
        super().__init__(entity_id, name, hub)
    ...
    def device_state_attributes(self):
        ...
        attr = {'Pushed': datetime.fromtimestamp(self._lastpushtime).strftime('%Y-%m-%d %H:%M:%S'),
                'Button': self._button,
