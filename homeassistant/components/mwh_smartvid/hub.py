        ...
        while time.time() < starttime + timeout:
            ...
            if len(self._discovereddevices) > 0:
                ...
                for device in self._discovereddevices:
                    #                    print(device)
                    self.devices.append(
                        SmartVidDevice(device["camera_id"], device["feedurl"], self)
                    )
                return True
    ...
    async def connect(self):
        def message_received(topic, payload, qos):
            ...
            if uid == "devices":
                ...
                print(
                    "Discovered {} smartvid devices:".format(
                        len(self._discovereddevices)
                    )
                )
                return
            ...
            if thisdevice is None and uid != "discover":
                print(
                    f"Error in SmartVid hub: didn't find device with uid {uid} loaded"
                )
                print(f"Loaded: {self.devices}")
            ...
            if len(payload) == 0:
                ...
                if "face" in args.keys():
                    ...
                    thisdevice._facetimestamp = datetime.now().strftime(
                        "%Y.%m.%d %H:%M:%S"
                    )
